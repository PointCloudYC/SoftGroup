...
def get_parser():
    ...
    parser.add_argument(
        '--data-root', type=str, default='/opt/code/SoftGroup/dataset/5gdataset/5GDataset', help='root dir save data')
    parser.add_argument( '--save-file', type=str, default='./class_numpoint_mean.csv', help='filename to store the class_numpoint_mean in the config of SoftGroup')
    # parser.add_argument(
...
if __name__ == '__main__':
    ...
    dict_instance_count = {name: 0 for i, name in INV_OBJECT_LABEL.items()}
    dict_instance_number_pts = {name: 0 for i, name in INV_OBJECT_LABEL.items()}
    ...
    for area_id in area_list:
        ...
        for room_name in room_name_list:
            ...
            for instance_name in instance_name_list:
                ...
                dict_instance_count[category_cur]+=1
                dict_instance_number_pts[category_cur]+=num_points-1


    print(dict_instance_count)
    print(dict_instance_number_pts)
    dict_instance_mean_points={x:int(dict_instance_number_pts[x]/dict_instance_count[x]) for x in dict_instance_count}
    print(dict_instance_mean_points)

    # https://blog.csdn.net/weixin_39750084/article/details/81429037
    df=pd.DataFrame(list(dict_instance_mean_points.items()))
